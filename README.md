...
2. Добавляете **клиента** → генерируются ключи, выдаётся свободный IP, `[Peer]` дописывается в конфиг сервера и применяется через `awg syncconf` (без разрыва соединений других клиентов).
3. На странице клиента — готовый конфиг, кнопка скачивания `.conf` и **QR-код** для приложения AmneziaWG на телефоне.
